...
- **v1 scope (by design)** — README lists streaming, JSON Schema subset,
  ...
  documented contracts, not missing features. M5 and A5 ship with this freeze;
  partial tool-call streaming, `unevaluated*`, and xAI OAuth defaults stay deferred.
...
- **P3 Train F — Flows (D-F1–D-F12)**: `Flow[S]` + `Start`/`Listen`/`Router`.
  Single-flight `ErrFlowRunning`; fail-fast default (ctx checked at next
...
- **P3 Train X — TraceRecorder (D-X1–D-X8)**: JSONL capture on Kickoff.
  Metadata-only default; `WithTraceBodies(true)` opt-in (redacted);
  `WithTraceFilter`; fold-order publish + KickoffID/wave; `Save` mode 0600.
  Example: `examples/trace_export`. Guide: `docs/training.md`.
...
- **P3 Phase 0 (complete)** — D-F1–D-F10 closed 2026-08-24
  ...
  closed 2026-09-16 (30 IDs total). Trains T, F, Y, and X shipped in this
  release. See `Plan/DECISIONS.md` §7B.
...
- **P3 design plan**: `Plan/PLAN.p3-flows-tools-yaml-training.md` (+ PT) —
  ...
  roadmap P3. Trains T, F, Y, and X shipped in this release.
