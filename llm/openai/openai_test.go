...
func TestClientCall(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"choices":[{"message":{"role":"assistant","content":"olá do mock"}}]}`))
	}))
	defer srv.Close()
	...
	out, err := c.Call(context.Background(), []crewai.Message{crewai.UserMessage("oi")})
	if err != nil {
...
func TestClientAPIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"error":{"message":"modelo inválido","type":"invalid_request"}}`))
	}))
	defer srv.Close()

	c := openai.New("x", openai.WithAPIKey("k"), openai.WithBaseURL(srv.URL))
	_, err := c.Call(context.Background(), []crewai.Message{crewai.UserMessage("oi")})
	if err == nil || !strings.Contains(err.Error(), "modelo inválido") {
		t.Errorf("erro = %v", err)
	}
}
