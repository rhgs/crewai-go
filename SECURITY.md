...
- **MCP.** Treat remote MCP servers as trusted code (see `docs/en/mcp.md` threat model). The client defaults to a 30s HTTP timeout (`DefaultHTTPTimeout`); override via `WithHTTPTimeout`, `WithHTTPClient`, or JSON `servers[].timeout` (`"0s"` disables). Protect JSON config files that hold bearer tokens (`0600`).
