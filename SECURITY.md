...
- **Trace export.** `TraceRecorder` is metadata-only by default (no prompt/output/tool args). `WithTraceBodies(true)` is opt-in and still redacts via `redactString`. `Save` writes JSONL with mode `0600` to a caller-trusted path.
