...
- **Stream sinks.** `Crew.WithStream` / `StreamFunc` receive **raw model text** deltas (and redacted terminal errors). Unlike `Progress`, stream content is intentionally the completion itself — filter/redact in the app before exposing to untrusted multi-tenant clients. `RedactHandler` does not wrap StreamFunc.
