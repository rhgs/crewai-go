...
- **SSRF.** `WebSearchTool` filters result URLs (non-http(s), userinfo, loopback/private/link-local/unspecified/multicast/CGNAT, DNS rebinding, fail-closed). It does **not** fetch those URLs — only presents them. `tools.HTTPFetch` **does** fetch: deny-by-default allowlist, GET-only default, max 3 redirects re-validated per hop, body cap `MaxToolOutputBytes`. Link-local / metadata hosts cannot be allowlisted. Never pass a model-controlled URL with an empty allowlist.
...
- **Trace export.** `TraceRecorder` is metadata-only by default (no prompt/output/tool args). `WithTraceBodies(true)` is opt-in and redacts prompt/output/fact-claim/tool-arg/tool-output strings via `redactString` (`payload_hash` is a hash, kept as-is). `Save` writes JSONL with mode `0600` to a caller-trusted path.
