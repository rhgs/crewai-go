...
- **P3 Trem X — TraceRecorder (D-X1–D-X8)**: captura JSONL no Kickoff.
  Metadata-only default; `WithTraceBodies(true)` opt-in (redigido);
  `WithTraceFilter`; publicação em ordem de fold + KickoffID/wave; `Save`
  modo 0600. Exemplo: `examples/trace_export`. Guia: `docs/training.md`.
