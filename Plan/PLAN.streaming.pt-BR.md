...
> **Decisões:** D-S1–D-S12 propostas abaixo (§9); fechar antes de codar a Fase 1.  
...
Caminhos não-stream usam `io.ReadAll(io.LimitReader(body, MaxProviderResponseBytes))`. Caminhos stream devem honrar o **mesmo teto de bytes no texto acumulado** (D-S8).
...
// v1 é centrada em texto: providers mapeiam deltas do wire em Delta.
type StreamChunk struct {
    ...
    // um fragmento final. Depois de um chunk Done o canal é fechado.
    Done bool
...
// StreamingLLM é uma capacidade opcional. O executor faz type-assert
// quando um sink de stream está configurado (D-S1, D-S3). Implementadores
// que só suportam Call continuam válidos para sempre.
type StreamingLLM interface {
    ...
    // ctx é cancelado. Implementadores DEVEM:
    //   - respeitar cancelamento de ctx com prontidão;
    //   - não bloquear para sempre se o caller parar de receber (preferir ctx);
    //   - aplicar MaxProviderResponseBytes no texto acumulado (D-S8);
    //   - ser seguros para Call/CallStream concorrentes no mesmo client.
    // O canal DEVE ter buffer pequeno (ex.: 16) ou ir pareado a uma
    // goroutine produtora que sai em ctx.Done.
...
// Retorna o primeiro chunk.Err non-nil, ou ctx.Err(), ou nil.
...
// StreamFunc é chamada para cada Delta non-empty (e opcionalmente Done).
// DEVE ser segura no goroutine chamador (o executor não faz fan-out próprio).
type StreamFunc func(StreamChunk)
```

`CollectStream` unifica o drain para providers e testes; o executor usa `CallOrStream` para LLMs sem stream não precisarem de mudança.
...
| Chunk | Quando | Notas |
|---|---|---|
| `{Delta: "Olá"}` | delta de texto do provider | pode ser multi-rune; sem garantia de fronteira de token |
| `{Delta: "!", Done: true}` | último texto + terminal | ou `{Done: true}` vazio separado — **D-S2**; recomendação: **permitir Delta final no chunk Done** |
| `{Err: err}` | falha HTTP/decode/teto | canal fecha em seguida |

Sem `Role`, sem parciais de tool-call, sem contagem de tokens na v1 (tipo estável e simples de redigir).

---

## 4. Integração no executor

### 4.1 Helper em todo lugar em que o texto é “final”
...
Substituir `a.LLM.Call` direto só nos caminhos em que streaming faz sentido.

### 4.2 Matriz de caminhos (D-S4)

| Caminho | Stream de deltas para o sink? | Mecanismo |
|---|---|---|
| ReAct **sem tools** | **Sim** | `CallOrStream` |
| ReAct **com tools** (turnos intermediários) | **Não** (v1) | `Call` puro — precisa do texto completo para parse Action/Final Answer |
| ReAct **com tools** (opcional depois) | adiado | só streamar turnos que já parecem Final Answer — fora da v1 |
| Loop nativo `CallWithTools` | **Não** nas rodadas de tool | manter `CallWithTools` |
| Resposta nativa só-texto final | **Sim** se houver stream-with-tools depois | **não na v1** |
| Structured output (todas as fases) | **Não** | validação precisa do JSON completo; loop de repair também |
| AgenticLoop plan / evaluate / refine | **Não** | completions curtas de controle |
| AgenticLoop sub-passo **execute** | segue `executeTaskDefault` | execute sem tools **pode** streamar |
| Hierarchical `delegate` | **Não** | completion minúscula; evita spam na UI |

**Resumo v1:** streamar o **caminho de resposta final visível ao usuário** que já é um `Call` full-text único (principalmente ReAct sem tools). Tudo protocol-driven permanece bufferizado.
...
func CallOrStream(ctx context.Context, llm LLM, messages []Message, sink StreamFunc) (string, error) {
    ...
    // Cortesia: se sink != nil, emite o texto inteiro como um Delta+Done
    // para a UI ainda atualizar uma vez (D-S7-B). Decisão: **sim**.
...
2. Escanear frames (SSE ou NDJSON); mapear para `StreamChunk{Delta}`.
3. Acumular texto em `strings.Builder`; se `builder.Len() > MaxProviderResponseBytes`, enviar `{Err: …}`, cancelar body, retornar.
4. Em EOF limpo / done do provider: enviar `{Done: true}`, fechar canal.
5. Produtor em goroutine; sair em `ctx.Done()` e fechar canal.
6. Nunca logar deltas completos em Info; Debug opcional sob orientação de redação existente.
7. Erros JSON do provider no meio do stream → chunk `{Err}`.
...
- xAI: herda via `inner *openai.Client` quando openai implementar `CallStream`.
...
| Tópico | Regra |
|---|---|
| **Teto de bytes** | Texto acumulado do stream ≤ `MaxProviderResponseBytes` |
...
- `CollectStream` concatena deltas; propaga Err; respeita cancel de ctx.
- `CallOrStream` com LLM não-Streaming + sink → um único Delta+Done.
- `CallOrStream` com sink nil → sempre `Call` (D-S9).
- Executor sem tools: sink recebe deltas ordenados; output final = concat.
- Executor com tools: sink **não** é chamado em turnos intermediários (v1).
- Mock stream + Kickoff + `WithStream` sob Sequential e uma wave Async (sink concurrency-safe via mutex).
- Provider httptest: fixture SSE OpenAI; NDJSON Ollama; SSE Anthropic; teto estourado → chunk Err.
...
- [ ] `StreamingLLM` + `StreamChunk` + `WithStream` / `ContextWithStream` públicos e com godoc
- [ ] Caminho sem tools streama com sink + LLM `StreamingLLM`
- [ ] Fallback de um chunk para LLMs sem stream
- [ ] openai + ollama + anthropic + xai (delegate) + mock implementam `CallStream`
- [ ] `examples/streaming` offline
- [ ] docs/llms.md + pt-BR + doc.go; checkbox no PLAN.md
...
| **D-S8** | Limite de tamanho | (A) teto novo maior (B) reusar `MaxProviderResponseBytes` | **B** |
| **D-S9** | sink == nil | (A) ainda CallStream se disponível (B) sempre Call | **B** |
| **D-S10** | Stream parcial de tool-call | (A) escopo v1 (B) adiado | **B** |
| **D-S11** | Buffer do canal | (A) unbuffered (B) 16 (C) 64 | **B** — 16 |
| **D-S12** | Panic em StreamFunc | (A) derruba Kickoff (B) recover + slog como Progress | **B** |

### 9.2 Abertas até spikes de implementação (não bloqueiam merge do plano)
...
| **O-S3** | `CollectStream` público vs interno | Recomendação: **público** |
...
```
Fase 0  Fechar D-S1–D-S12 neste documento (este PR / review)
Fase 1  S1: API stream.go + CollectStream/CallOrStream + testes (sem provider)
Fase 2  S2: llm/mock CallStream + wiring no executor sem tools + Crew.WithStream
Fase 3  S3: llm/openai CallStream (httptest SSE) + assert xai via inner
...
| `examples/streaming` | Mock offline imprimindo deltas |
...
Shippar **`StreamingLLM` opcional** + **`Crew.WithStream` / sink no context**, reutilizar o **padrão de type-assert de `ToolCallingLLM`**, streamar **só caminhos de texto final** na v1, **fallback para Call de uma vez** (com um chunk se houver sink), implementar **mock → openai/xai → ollama → anthropic**, manter **Progress sem corpos** e **só-stdlib**.

Quando D-S1–D-S12 forem reconhecidas, a Fase 1 de código pode começar em um branch como `feat/streaming-s1-api`.
